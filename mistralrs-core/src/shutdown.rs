...
impl Shutdown {
    pub(crate) fn enter(&self) -> Result<Admission<'_>, MistralRsError> {
        if ENGINE_THREAD.with(Cell::get) {
            return Err(MistralRsError::Shutdown(engine_thread_error()));
        }
    ...
    pub(crate) fn run(
        ...
        if ENGINE_THREAD.with(Cell::get) {
            return Err(engine_thread_error());
        }
        let mut state = self.state.lock().expect("engine shutdown state lock");
        loop {
            match &state.phase {
...
fn engine_thread_error() -> MistralRsShutdownError {
    MistralRsShutdownError(vec![MistralRsShutdownFailure::EngineThreadCannotJoin].into())
}
...
pub(crate) fn engine_runtime() -> std::io::Result<tokio::runtime::Runtime> {
    tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .on_thread_start(|| ENGINE_THREAD.set(true))
...
impl<T> RetiringEngine<T> {
    pub(crate) fn new(sender: Sender<Request>, worker: JoinHandle<()>, resources: T) -> Self {
        Self {
            sender,
            worker,
            resources,
...
fn into_retiring(engine: EngineInstance) -> RetiringEngine<impl Send> {
    let EngineInstance {
        sender,
        engine_handler,
        reboot_state,
    ...
    RetiringEngine::new(
        sender,
        engine_handler,
        (reboot_state, config, category, logger),
    )
}
